...
const target_os = process.argv[5]
...
let new_context = context.slice(0, run_mksnapshot_start) + context.slice(run_mksnapshot_start, run_mksnapshot_end).replace("$current_cpu", target_cpu).replace("--target_os=$current_os", `--target_os=${target_os}`) + context.slice(run_mksnapshot_end)
